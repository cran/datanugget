refine.DN = function(x,
  ...
  if (class(DN) != "datanugget"){

    stop('DN must be of class "datanugget"')

  }

  # make sure scale.tol is of class numeric
  if (class(scale.tol) != "numeric"){

    stop('scale.tol must be of class "numeric"')

  }

  # make sure scale.tol is within (0,1)
  if (scale.tol >= 1 |
      scale.tol <= 0){

    stop('scale.tol must be within (0,1)')

  }

  # make sure shape.tol is of class numeric
  if (class(shape.tol) != "numeric"){

    stop('shape.tol must be of class "numeric"')

  }


  # make sure shape.tol is within (0,1)
  if (shape.tol >= 1 |
      shape.tol <= 0){

    stop('shape.tol must be within (0,1)')

  }

  # make sure min.nugget.size is of class numeric or integer
  if (class(min.nugget.size) != "numeric" &
      class(min.nugget.size) != "integer"){

    stop('min.nugget.size must be of class "numeric" or "integer"')
  ...
  # make sure max.nuggets is of class numeric
  if ((class(max.nuggets) != "numeric" &
       class(max.nuggets) != "integer") |
      max.nuggets <= nrow(DN$`Data Nuggets`)){

    stop('max.nuggets must be of class "numeric" or "integer" and greater than the of data nuggets in argument "DN".')

  }

  # convert min.nugget size to an integer
  max.nuggets = floor(max.nuggets)

  # make sure min.nugget.size is greater than 1 is of class datanugget
  if (min.nugget.size < 2){

    stop('min.nugget.size must be an integer greater than 1')

  }

  # make sure scale.max.splits is of class numeric or integer
  if (class(scale.max.splits) != "numeric" &
      class(scale.max.splits) != "integer"){

    stop('scale.max.splits must be of class "numeric" or "integer"')

  }

  # convert scale.max.splits to an integer
  scale.max.splits = floor(scale.max.splits)

  # make sure shape.max.splits is of class numeric
  if (class(shape.max.splits) != "numeric" &
      class(shape.max.splits) != "integer"){

    stop('shape.max.splits must be of class "numeric" or "integer"')

  }

  # convert shape.max.splits to an integer
  shape.max.splits = floor(shape.max.splits)
  ...
  # retrieve the original threshold for scale
  orig.threshold = as.numeric(quantile(DN.information[DN.information[, "Scale"] != 0, "Scale"],
                                       probs = scale.tol))

  # retrieve the data nuggets with scales larger than the scale tolerance
  large.scales = DN.information[which(DN.information[, "Scale"] > orig.threshold), "Data Nugget"]

  # retrieve the amount of data nuggets with large scales
  cur.LS.num = length(large.scales)

  # initialize new amount of data nuggets with large scales
  ...
  message("splitting data nuggets according to scale...")

  # perform the following operations while the number of data nuggets with large scales is
  # both > 0 and different from the previous number
  while(cur.LS.num > 0 &
        new.LS.num > 0 &
        cur.LS.num != new.LS.num &
        nrow(DN.information) < max.nuggets &
        split.attempts < scale.max.splits){
    ...
    # reset the current ammount of data nuggets with large scales
    cur.LS.num = new.LS.num
    ...
    if (no.cores != 0){
      ...
      if (make.pbs == TRUE){
        ...
        pb = txtProgressBar(min = 0,
                            max = length(large.scales))

        # update the progress bar
        progress = function(n){setTxtProgressBar(pb, n)}
        opts = list(progress = progress)

        # split data nuggets with large scales
        tmp.new.DN.information = foreach(i = large.scales,
                                         .combine = rbind,
                                           ...
                                           if (!is.null(ncol(x))){
                                             ...
                                             if (nrow(assign.obs) >= 2*min.nugget.size){
                                               ...
                                               if (nrow(assign.obs) > 2){
                                                 ...
                                                 tmp.DN.assignments[assign.locs] = shift + 2*(which(large.scales == i)-1) + new.nugget.info$cluster

                                                 # initialize data frame for new data nuggets
                                                 new.DN.information = DN.information[1:2, ]

                                                 # create new data nugget names
                                                 new.DN.information[1:2, "Data Nugget"] = shift + 2*(which(large.scales == i)-1) + 1:2
                                                 ...
                                                 # retrieve the scale for the new data nuggets
      ...
      if (make.pbs == TRUE){
        ...
        # cycle through the data nuggets with large scales
        for (i in large.scales){
    ...
    # retrieve the data nuggets with scales larger than the scale tolerance
    large.scales = DN.information[which(DN.information[, "Scale"] >
                                          orig.threshold), "Data Nugget"]

    # retrieve the amount of data nuggets with large scales
    new.LS.num = length(large.scales)
  ...
  # check if the shape of the data nugget is relevant
  if (!is.null(ncol(x))){

    # check if user wants to use parallel processing
    if (no.cores != 0){

      # find the data nuggets with the widest shapes
      tmp.shape.ratios = foreach(i = 1:nrow(DN.information),
                                 .combine = c)  %dopar%

                                 {
                                   ...
                                   if (length(assign.locs) > 1){
                                     ...
                                     # retrieve the top eigenvalue ratios
                                     shape.ratio = eigen.values[1]/eigen.values[2]
      ...
      # initialize the vector of data nugget shapes
      tmp.shape.ratios = NULL
    ...
    # retrieve the original threshold for scale
    orig.threshold = as.numeric(quantile(tmp.shape.ratios[tmp.shape.ratios!= 0],
                                         probs = shape.tol))

    # retrieve the data nuggets with shapes larger than the shape tolerance
    large.shapes = which(tmp.shape.ratios > orig.threshold)
    ...
    while(cur.LS.num > 0 &
          ...
          cur.LS.num != new.LS.num &
          nrow(DN.information) < max.nuggets &
          split.attempts < shape.max.splits){
